...
def get_AV(model, loader, device, centers=None):
    """提取模型推理结果，包括 OpenMax 所需的 z_raw 激活向量"""
    model.eval()
    res = {
        'embs': [], 'c_true': [], 'f_true': [], 'snr': [],
        'c_pred': [], 'f_pred': [], 'max_sim': [],
        'z_raw_list': [],    # <-- OpenMax 核心：未归一化的激活向量
        'coarse_logits': [], # <-- OpenMax 核心：粗分类 logits
    }
    # 记录辅助诊断维度
    raw_metrics = {
        'msp': [],      # 最大 softmax 概率
    ...
    with torch.no_grad():
        for x, y_c, y_f, snr in tqdm(loader, desc="Inference"):
            ...
            res['embs'].append(output['embedding'].cpu().numpy())          # z_norm, [B, 128]
            res['z_raw_list'].append(output['z_raw'].cpu().numpy())        # <-- OpenMax: z_raw, [B, 128]
            res['coarse_logits'].append(c_logits.cpu().numpy())            # <-- OpenMax: [B, num_coarse]

            # --- 2. MSP (Maximum Softmax Probability) ---
            probs = torch.softmax(c_logits, dim=1)  # [B, num_coarse]
            msp, _ = torch.max(probs, dim=1)         # [B]
            raw_metrics['msp'].append(msp.cpu().numpy())

            # --- 3. 归一化熵 (除以各类均匀分布时的最大熵) ---
            ent = -torch.sum(probs * torch.log(probs + 1e-10), dim=1)  # [B]
            num_coarse_cls = probs.shape[1]
            raw_metrics['entropy'].append((ent / np.log(max(num_coarse_cls, 2))).cpu().numpy())

            # --- 4. ArcMargin 最大相似度 (辅助诊断) ---
            z = F.normalize(output['embedding'], p=2, dim=1)       # [B, 128]
            prototypes = F.normalize(model.fine_head.weight, p=2, dim=1)  # [num_fine, 128]
            sim_matrix = torch.matmul(z, prototypes.t())           # [B, num_fine]
            max_sim, _ = torch.max(sim_matrix, dim=1)              # [B]
...
def main():
    ...
    # 在训练集上推理，提取 z_raw
    train_res = get_AV(model, train_loader, cfg.device)

    # 收集每个已知类的所有训练样本的 z_raw (训练集只有已知类，无需预测正确掩码)
    class_activations = {}
    for i in range(num_coarse):
        mask = train_res['c_true'] == i
        class_activations[i] = train_res['z_raw_list'][mask]
        print(f"  类别 {i} ({COARSE_LABEL_NAMES[i]}): {mask.sum()} 个训练样本用于 MAV 计算")

    # 初始化并拟合 OpenMax 校准器 (MAV + Weibull)
    calibrator = OpenMaxCalibrator(tail_size=20)
    calibrator.fit(class_activations, num_coarse)

    # 在训练集上计算 OpenMax unknown 概率，用于设定参考阈值 (仅可视化用)
    train_openmax_unknown = []
    for i in range(len(train_res['c_true'])):
        z_raw_i = train_res['z_raw_list'][i]
        logits_i = train_res['coarse_logits'][i]
        probs, _, _ = calibrator.predict_openmax(z_raw_i, logits_i)
    ...
    # 在测试集上推理
    test_res = get_AV(model, test_loader, cfg.device, centers=centers)

    known_mask_test = (test_res['c_true'] != -1)
    unknown_mask_test = (test_res['c_true'] == -1)

    # 应用校准好的 OpenMax 模型，使用 argmax 判定 (无需阈值)
    openmax_unknown_probs_test = []
    openmax_preds_test = []  # OpenMax 直接预测 [0..num_coarse-1]=已知类, num_coarse=unknown

    for i in range(len(test_res['c_true'])):
        z_raw_i = test_res['z_raw_list'][i]
        logits_i = test_res['coarse_logits'][i]
        probs, _, _ = calibrator.predict_openmax(z_raw_i, logits_i)
        openmax_unknown_probs_test.append(probs[-1])
        openmax_preds_test.append(np.argmax(probs))

...
def main():
    ...
    # --- 4. 保存训练集数据 (仅已知码，用于 OpenMax MAV 校准) ---
    train_data_for_eval = {
        'features': x_train,
        'coarse_labels': y_coarse_train,
        'fine_labels': y_fine_train,
        'snrs': snrs_train,
        'coarse_map': coarse_map,
        'fine_map': fine_map,
        'num_known_train': len(x_train)
    }
    train_path = EVAL_DATA_PATH.replace('.pkl', '_train.pkl')
    with open(train_path, 'wb') as f:
        pickle.dump(train_data_for_eval, f)
    print(f"Training data (Known only) saved to {train_path} for OpenMax MAV calibration.")

    # --- 5. 训练集的 DataLoader (仅已知码) ---
